...
class Runner {
    ...
    async create(config) {
        ...
        } else if (config.browser.selenium) {
            const SeleniumDriver = require('./selenium');
            this.driver = new SeleniumDriver(config.browser, this.logger);
        } else throw new Error('No browser driver specified');
    ...
    async run(config) {
        ...
        config.exitPaths = config.exitPaths.map(path => {
            ...
            return {
                host: pathUrl.host,
                path: pathUrl.path,
                pathRe: pathToRegExp(pathUrl.pathname)
            };
        });

        const lifecycle = new Lifecycle(config, this.driver, this.logger, this.values);
        ...
        try {
            ...
            if (data.errorElementContent) this.logger.error('Error element content:', data.errorElementContent);
