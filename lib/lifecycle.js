...
class LifeCycle {
    ...
    async capture(pageUrl) {
        ...
        await this.driver.caputreHtml(filename + '.html');
    ...
    findPageConfig(pageUrl) {
        ...
        let pageConfig = _.find(this.config.pages, value => value.pageRegExp.test(pageUrl.path));
    ...
    checkIfExitPage(pageConfig) {
        ...
        let exitPage = _.find(this.config.exitPaths, path => {
            return pageConfig.url.host === path.host && path.pathRe.test(pageConfig.url.path);
        });
        if (exitPage) {
    ...
    async processPage(pageUrl) {
        ...
        if (pageConfig.viewport) await this.driver.setViewport(pageConfig.viewport);
