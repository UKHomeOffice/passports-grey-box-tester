...
class LifeCycle {
    ...
    toJSON() {
        return {
            ...
            errorElementContent: this.errorElementContent
        };
    }
    ...
    async checkIfErrorPage(pageConfig) {
        ...
        for (let selector of pageConfig.errorPages) {
            ...
            if (isError) {
                this.errorElementContent = (await this.driver.getElementValue(selector)).split('\n');
                throw new Error(`Error element ${selector} found at ${pageConfig.url.href}`);
            }
        }
    }
    ...
    async processPage(pageUrl) {
        ...
        try {
            ...
            if (typeof pageConfig.navigate === 'string') {
                ...
                let nextURL = new url.URL(pageConfig.navigate, this.config.url.href);
    ...
    async run() {
        ...
        try {
            ...
            pageUrl = new url.URL(await this.driver.getUrl());
